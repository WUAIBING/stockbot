...
# 默认输出目录（Windows兼容）
_output_base = os.path.expanduser('~/.workbuddy/a-share-analyst')
...
def format_result(intent, result):
    ...
    if intent == 'balance':
        ...
        currency_unit = data.get('currencyUnit', 1) or 1
        total_assets = data.get('totalAssets', 0) / currency_unit
        avail_balance = data.get('availBalance', 0) / currency_unit
        frozen_money = data.get('frozenMoney', 0) / currency_unit
        total_pos_value = data.get('totalPosValue', 0) / currency_unit
        total_pos_pct = data.get('totalPosPct', 0)
        nav = data.get('nav', 0)
        init_money = data.get('initMoney', 0) / currency_unit
        ...
        total_profit = data.get('totalProfit', 0) / currency_unit
